...
merged_df %>% 
  as.numeric(hh_income) %>% 
  mutate(hhinc_med = if_else(hh_income > median(hh_income), "above", "below")) %>% 
  group_by(hhinc_med = if_else(hh_income > median(as.numeric(merged_df$hh_income)),"above", "below")) %>%
             summarize(mean_crime = mean(crime),
                       mean_loans = mean(loans))

merged_df %>% 
  group_by(above_avg = (hh_income > median(hh_income)),
           below_avg = (hh_income < median(hh_income)))  %>%
             summarize(mean_crime = mean(crime),
                       mean_loans = mean(loans))

merged_df %>% 
  group_by(
    hhinc_med = ifelse(hh_income > median(hh_income, na.rm = TRUE), "above", "below")) %>% 
