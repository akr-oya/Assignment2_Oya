...
wide_df %>%   
  arrange(desc(loans)) %>% #sort loans column in descending order
  head()
...
#If I wanted to see the data sorted by crime
# wide_df %>%   
#  arrange(desc(crime)) %>% #sort crime column in descending order
#  head()
```
...
```
While `loans` had a higher mean search frequency over time, there does not seem to be a large difference between the search frequency of `crime`. However, patterns can be seen in the plot. The two keywords may have an inverse relationship where search frequencies for `loans` tend to be high when `crime` is low, and vice versa. We tested the relationship between the variables for interest by city to properly with a Pearson correlation test. The test suggests that there is no significant correlation (r = -0.115, p = .72). \
...
```{r}
# Reshape data from long to wide format using keywords
wide2_df <- res2$interest_by_city %>% 
  pivot_wider(names_from = keyword,
              values_from = hits)
head(res2$interest_by_city)
...
wide2_df %>%   
  arrange(desc(masks)) %>% #sort masks column in descending order
  head()
