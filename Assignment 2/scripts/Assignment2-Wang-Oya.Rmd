...
res$interest_over_time %>% 
  ...
  summarize(mean_hits = mean(hits),
            ...
            var_hits = var(hits))
```
The keyword "crime" produced a mean search hit of 55.615 with a standard deviation of 9.282 and a variance of 86.163. The keyword "loans" produced a mean search hit of 67.231 with a standard deviation of 9.924 and a variance of 98.495.\
...
# Reshape data from long format to wide format using keywords
res$interest_by_city %>% 
  pivot_wider(names_from = keyword,
              values_from = hits) %>% 
  glimpse() %>% 
  arrange(desc(loans)) #sort loans column in ascending order
# Results in many NAs in loan column

```
- The city of Clinton, IL has the highest search hits for the keyword "loans" with 44 hits. All other cities had missing search hit values.\
